    ...
    for (const { cmd, type, args } of commands) {
        try {
            if (platform === 'windows') {
                ...
                console.log(`[OK] 检测到剪贴板工具: ${cmd} (${type})`);
...
async function setClipboard(text) {
    ...
    try {
        ...
        console.log(`[OK] 已同步到系统剪贴板 (${type}): ${text.substring(0, 50)}${text.length > 50 ? '...' : ''}`);
        return true;
    } catch (err) {
        console.log('[警告] 系统命令失败，尝试 clipboardy...');
...
function connect(serverUrl, roomId) {
    ...
    try {
        ...
        ws.on('close', (code, reason) => {
            ...
            console.log(`[错误] 连接已断开 (代码: ${code}, 原因: ${reason || '未知'})`);
            ...
            reconnectTimer = setTimeout(() => {
                ...
                console.log(`[房间] 重连房间: ${savedRoomId || '主房间'}`);
...
async function main() {
    ...
    console.log('╚═══════════════════════════════════════╝');
    console.log('');
    console.log(`[提示] 服务器地址: ${SERVER_URL}`);
    console.log('');
    ...
    process.on('uncaughtException', (err) => {
        console.error('\n[错误] 未捕获的异常:', err.message);
        console.error('   堆栈:', err.stack);
        // 不退出进程，继续运行
    });

    process.on('unhandledRejection', (reason, promise) => {
        console.error('\n[错误] 未处理的Promise拒绝:', reason);
        // 不退出进程，继续运行
    });
}
